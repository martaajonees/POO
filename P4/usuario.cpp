...
void Usuario::compra(const Articulo& arti, unsigned int cantidad){
    Articulo *art = const_cast<Articulo*>(&arti);
    auto it = articulos_.find(const_cast<Articulo*>(art)); //Miramo si existe
    if (it == articulos_.end()){ //No está
        if(LibroDigital* tipo = dynamic_cast <LibroDigital*>(art)){
            if (tipo->f_expir()>=Fecha()){
                articulos_.insert(std::make_pair(const_cast<Articulo*>(&arti), cantidad));
            }
		} else{
            articulos_.insert(std::make_pair(const_cast<Articulo*>(&arti), cantidad));
        }
...
ostream& mostrar_carro(ostream& os, const Usuario& user){
    ...
    for(auto art: user.compra()){
        os<<"["<<art.first->referencia()<<"] \""<<art.first->titulo()<<"\", "<< art.first->f_publi().anno()<<". "
        <<std::setprecision(2)<< std::fixed<< art.first->precio()<< " €"<<endl;
    }
    return os;
}
